...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
    ...
    for (var key in params) {
        if (key != nsAttrName) {
            ...
            // part of a dirty hack, remove rather sooner then later
            if (key !== 'parameters') {
                parts.push(['<',key,'>'].join(''));
            } else if (typeof(value) !== 'undefined' && value.length > 0) {
                parts.push(['<soapenc:Array soapenc:arrayType="xsd:string[' + value.length + ']" type="soapenc:Array">'].join(''));
            }
...
//            }
            // this is a very dirty hack, need to fix this rather sooner then later
            parts.push(['<item type="xsd:string">'].join(''));
...
WSDL.prototype._xmlnsMap = function() {
    ...
    str += ' xmlns:soapenc="http://schemas.xmlsoap.org/soap/encoding/"';
    str += ' xmlns:wsdl="http://schemas.xmlsoap.org/wsdl/"';
